...
class AttentionModel():
    ...
    def ensemble(self, experiment, class_weight=None, freeze = True, train=True):
        ...
        if self.config["train"]["gpus"] > 1:
            with self.strategy.scope():        
                self.ensemble_model = Hang.metadata_ensemble(HSI_spatial=self.HSI_spatial, HSI_spectral=self.HSI_spectral, metadata_model= self.metadata_model, classes=self.classes)
